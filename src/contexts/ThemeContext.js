...
export function ThemeProvider(props) {
  ...
  const [themeOptions, dispatch] = React.useReducer(
    (state, action) => {
      switch (action.type) {
        ...
        case "CHANGE":
          localStorage.setItem("paletteMode", action.payload.paletteMode);
          return {
            ...state,
  ...
  React.useEffect(() => {
    if (typeof window !== "undefined") {
      let paletteMode = localStorage.getItem("paletteMode");
      if (!paletteMode) {
...
/**
 * @returns {(options: Partial<typeof themeInitialOptions>) => void}
 */
